  ...
  return (
    <>
      <div className="flex justify-center gap-x-10">
        <Link to={`/playlists/${id}/?limit=10&offset=${Number(offset) - 10}`}>
          <button disabled={data.playlists.previous === null}>previous</button>
        </Link>
        <Link to={`/playlists/${id}/?limit=10&offset=${Number(offset) + 10}`}>
          <button disabled={data.playlists.next === null}>next</button>
        </Link>
      </div>
      <AnimatePresence>
        {!exit && (
